...
The merged dataset exploration revealed missing values (NA), special
codes (7,9,77,99) and considering special codes are not random and
cannot be dropped, based on variable (codebook) they were transformed
into NAs. Missing values (NAs) were included in the analysis, since R
automatically excludes rows with NA via listwise deletion or complete
case analysis during regression.

On running lm (), listwise deletion of rows, resulted in reduced sample
size (n=14) which we acknowledged could introduce bias as the
informative missingness is ignored (MAR or MNAR). The regression
resullted in quasi-sepration (warning).
...
-   The bar plots in Figure 4 show frequency distributions for
    categorical variables (BMI, gender, diabetes status, race). Findings
    revealed most cases are non-hispanic whites. Of those who reported
    diabetes status, more counts reported having diabetes. Genders are
    relatively evenly distributed. Majority population were in the
    normal BMI range.
-   Histograms in Figure 5 show age (continuous) frequency distributions
...
The merged data variables were recoded and special codes were included
in NA category. The descriptive summary of the merged data are presented
in tabulated format for continuous and categorical variables (counts,
frequencies, proprotions mean and sd)
...
We conducted Multiple linear regssion on our dataset and we see the
(presented are first 6 deleted rows) listwise deletion resulted in small
sample size.

After complete case analysis, listwise deletion of NAs, resulted in
perfect (or quasi-perfect) separation. @van2012flexible. The deatils of
the missingness (plot below) revealed the cause of missingness (MAR and
MNAR) which we found are common in healthcare and public health
datasets.

**Firth (penalized) regression**

Since **Firth (penalized) regression** handles quasi-separartion,
@DAngelo2025, we conducted firth regression. Results from Firth
regression conducted for quasi sepration shows all coefficients as
finite, but the sample size was reduced (n= 14) where estimates are
highly uncertain, wide confidence intervals → cannot make strong claims
about predictor effects.
...
-   Firth regression - coefficients as finite, but the sample size was
    reduced (n= 14) where estimates are highly uncertain, wide
    confidence intervals → cannot make strong claims about predictor
    effects.
    -   Firth regressionis a frquestist approach where Jeffreys prior is
        ...
        regression). To avoid sample loss and potential bias in
        subsequent analyses, we considered multiple imputation (MI) as
        an alternative approach for the given dataset.
...
multimodality do not appear to hamper the good performance of multiple
...
-   A heatmap of the imputed dataset generated correlation between **BMI
    categories** and **Diabetes status.** BMI dummy variables are
