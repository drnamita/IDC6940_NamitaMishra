...
The NAs were removed (listwise deletion) from the merged data. Clean
data with reported complete cases (n=14).
...
Special codes in merged data were recoded, while NAs were included in
the dataset. Variables were named accordingly and we performed multiple
linear regression on merged (raw dataset).
...
-   We found quasi-complete separation issue (with 9799 observations
    deletion and resulting complete cases (n=14). The original data
    could not be analysed further because of "complete separation” or
    “quasi-complete separation”. With sparse data as some categories had
    very few observations, the resulting imbalance in the outcome (very
    few cases of 0 or 1) it emphasized strong predictors that almost
    perfectly separate outcomes,d and the combination resulted in empty
    cells on cross-tabulation.
...
-   Visulaization of the original data -

    -   Cross-tabulation between BMI and diabetes status resulted only
        14 complete cases with higher number reporting having diabetes
        (10/14) -

    -   Box-plot for age and diabetes status - higher number of
        population had diabetes in the age range of \< 20 years.

    -   Age distribution presented in the graph.
...
-   Because of quasi complete data issue, Multiple Imputation (MI)
    performed on merged data for missingness in the predictor (BMI) and
    in the response variable (diabetes status).
-   A Bayesian Approach in the popular mice package in R to add sampling
    variability to the imputations, Iterative Imputation (MICE) imputes
    missing values one variable at a time, using regression models based
    on the other variables in the dataset. This chain process, with each
    imputed variable becoming a predictor for the subsequent imputation
...
-   Imputation resulted in filling imputed values and resulted dataset
    had 9813 observations with no NAs. The imputed data was analyzed and
    summarised as below.
-   A comparative bar plot of the original merged data and the imputed
    data is presented below.
-    A heatmap generated correltaion between **BMI categories** and
    **Diabetes status.** BMI dummy variables are strongly **negatively
    correlated and there is** no strong linear association between BMI
    category and diabetes in your dataset. Since bothe being categorical
    the chi-square calculation shows (p-value = 0.5461) is \> 0.05 with
...
In our dataset, we found challenges such as:

-   Small sample size with very few diabetics in the "Underweight" BMI
    group.

-   Logistic regression estimates may give huge standard errors.

-   Rare events such as imbalanced outcome MLE estimation can be biased.

-   We found separation problem in the data after listwise deletion.

-   Our data resulted in a predictor perfectly predicting diabetes
    (e.g., all obese participants are diabetic in your subset), where
    frequentist regression fails.

Bayesian data augmentation and MCMC sampling

-   We found Bayesian augmentation is best analytic procedure for our
    data. Bayesian methods shrink extreme estimates back toward
    plausible values.

-   Augmentation stabilizes estimates via priors. Bayesian augmentation
    with priors provides finite, interpretable estimates. Encoding prior
    belief could help in predicting estimates when data are sparse.
    Bayesian augmentation could help prevent unstable estimates and
    provide more reliable inferences.

-   It gives a full probability statements instead of only Odds ratio.
    Bayesian provides 94% probabilitic results (credible intervals), not
    just p-values. Prior knowledge matters as literature already
    suggests obesity is linked to diabetes.

-   On comparing MI vs Bayesian data augmentation and MCMC sampling. MI
    is a frequentist approach, requires no priors, has moderate
...
-   We conduct Bayesian data augmentation and MCMC sampling from the
    joint posterior (both regression coefficients and the missing
    values) for Bayesian logistic regression.

-   Bayesian data augmentation performs both the and the regression
    model fitting happening simultaneously, and Markov Chain Monte
...
-   This technique handles uncertainty in missing values fully
    ...
    datasets and separation problems. It includes informative priors on
    coefficients as well.

##    Modeling 

(1) We first performed MI and then run multiple logistic regression on
    imputed dataset (MI) as above.

(2) Next, we conduct bayesian data augmentation and logistic regression.

(3) Lastly, we compare model developed on raw data, imputed (MICE) data
    and Bayesian augmention model.

    ## **Results** 

We present key findings Bayesian data augmentaton and regression through
visualization

(1) Modeling assumption check
(2) Correlation matrix, cooks distance, influential points
(3) Model plot
...
Findings from regression of MI data set and Bayesian data augmentation:
...
2.  Bayesian Data Augmentation and logistic regression approach

-   We incorporate prior knowledge that BMI increases diabetes odds by
    .,
-   We use priors and Bayesian logistic regression and compare the
    models with different priors in the model
    -   Weak prior N (0,2.5)
    -   informative prior from previous studies N(
