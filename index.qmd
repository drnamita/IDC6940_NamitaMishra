...
The model provided an estimate of the posterior distribution for each
...
patient population, reduced the risk of over-treatment, complications,
...
especially given the limited existing literature, and are more robust in
...
questionnaire) from year 2013-2014.
...
power. the descriotives of and proper handling of missing values was conducted. Final Analytic
dataset included binary response variable (Diabetes), predictor variables
(BMI), and covariates (age, gender, and race). Multiple
...
• Participants missing CVD response in all MCQ160 series were merged
into NO. • Participants with missing in smoking_cat were added under
“Never” to preserve the row numbers. • CDQ009F (Pain in left chest)
column, with a high % of missingness, was excluded to avoid overlap with
the CVD response.
...
1.  Response Variable
    ...
    CVD - Composite cardiovascular disease outcome (CVD- binary) is
    derived by combining MCQ series("MCQ160B", "MCQ160C" ,
    "MCQ160D","MCQ160E","MCQ160F"•

    -   Ever told had congestive heart failure MCQ160B

    -   Ever told you had coronary heart disease MCQ160C

    -   Ever told you had angina/angina pectoris MCQ160D

    -   Ever told you had heart attack MCQ160E

    -   Ever told you had a stroke MCQ160F

        The variable CVD, was transformed from the original values (Yes
        or no) and recoded into numeric and binary (1 = any “Yes”, 0 =
        all “No”, NA if all missing).

    Predictor Variables

    -   Smoking Variable (SMQ030) was categorized from numeric format in
        the original dataset (SMQ030) to categories based on the age (in
        years) of starting smoking regularly. The variable has 3 levels
        (smoking_cat):

        -   Never smoker: SMQ030 = NA

        -   Early starter: SMQ030 ≤ 18 years

        -   Late starter: SMQ030 \> 18 years

            We found it rationale to have missing under NAs, as
            participants who never smoked do not report an age (Never
            Smokers), and those who smoked were further categorized into
            early vs. late starters based on age considering that it
            allows better assessment of age at initiation as a risk
            factor and can be included in logistic regression model,
            with “Never smoker” as the reference group.
...
-   The age showed a skewed histogram. Most population were from
    ...
    years
-   Smoking age when first started regularly shows a higher number in \<
    20 years range
-   lab report on total cholesterol had a mean of 4.53 (mmol/L) with max
    being 21 (mmol/L)
...
-   Majority reported never started smoking followed by the age of
    starting smoking regularly in the range of early age (\<18 years)
    category.
-   Total number of population being told having CVD (stroke, coronary
    disease, heart attack, infarction) reported "NO"
