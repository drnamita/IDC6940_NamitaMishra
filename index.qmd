...
**Firth (penalized) regression** handles quasi-separation, @DAngelo2025,
and the results from Firth regression shows all coefficients as finite,
but sample size was reduced (n= 14). The estimates were highly
uncertain, and have wide confidence intervals that cannot make strong
...
**Challenges** observed in the original data

-   Small sample size with very few diabetics in the "Underweight" BMI
    group.

-   Logistic regression estimates with huge standard errors.

-   Imbalanced outcome MLE estimation, which can be biased.

-   Quasi-separation in the data after listwise deletion.

-   Data with a predictor perfectly predicting diabetes (e.g., all obese
...
**Firth regression (raw data) -**

-   coefficients as finite, but the sample size was reduced (n= 14)
    where estimates are highly uncertain, wide confidence intervals →
    cannot make strong claims about predictor effects.
-   Firth regressionis a frquestist approach where Jeffreys prior is
    added for bias correction. It does not provide posterior and no
    sampling using MCMC (as in contrast to bayesian logisitic
    regression). To avoid sample loss and potential bias in subsequent
    analyses, we considered multiple imputation (MI) as an alternative
    approach for the given dataset.

**Multivariate Imputation by Chained Equations (MICE)**

Flatness of the density, heavy tails, non-zero peakedness, skewness and
multimodality do not hamper the good performance of multiple imputation
for the mean structure in samples n \> 400, even for high percentages
(75%) of missing data in one variable \@@van2012flexible. We performed
Multivariate Imputation by Chained Equations (MICE) @JSSv045i03

Multiple Imputation (MI) on raw data was considered to handle
missingness in the predictor (BMI) and in the response variable
(diabetes status).
...
Iterative Imputation (MICE) imputes missing values of one variable at a
time, using regression models based on the other variables in the
dataset. This is a chain process, with each imputed variable becoming a
...
reflecting different possibilities for the missing data. Each variable
is imputed using its own appropriate univariate regression model.

**Results from MICE:**

-   Performing MI on raw data resulted in filling imputed values with
    resulting 9813 observations with no NAs. A comparative bar plot on
    missingness in the raw data and the imputed data is presented below.
...
-   Initially we conducted a Linear regression model on raw data

-   To overcome perfect fitting issues we conducted Firth regression.

-   To resolve the small sample size, we performed MI and conducted
    multiple logistic regression on the ,imputed dataset

-   Next, we conduct bayesian data augmentation and logistic regression
...
For the given dataset, we use Bayesian logistic Regression (GLM) to
...
To overcome small sample size and the other challenges in the raw data,
we conducted Bayesian data augmentation and MCMC sampling

-   Bayesian augmentation stabilizes estimates via priors and provides
    finite, interpretable estimates and provides more reliable
    inferences.
...
-   Bayesian gives full probability statements instead of only Odds
    Ratio (OR) with 94% probabilistic results (credible intervals), and
    not just p-values.

    **Comparing MI vs Bayesian data augmentation with MCMC sampling**

-   **MI** is a frequentist approach, requires no priors, has moderate
    flexibility, it handles missing values first by imputation, performs
    regression analysis, pools results, and propagate uncertainty added
    after analysis (pooling).
...
-   **Bayesian data augmentation** performs missing data imputation and
    ...
    ## **Results from** Bayesian data augmentaton and regression

Key findings from Bayesian data augmentaton and regression through
visualization
